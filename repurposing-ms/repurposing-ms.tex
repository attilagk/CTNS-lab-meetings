...
Drugs with low values of network proximity to AD genes are good candidates for
repurposing to AD.  We performed a network proximity based drug repurposing
screen on 2413 approved or phase 3 drugs (Fig.~\ref{fig:workflow}A).  For each
drug we computed eight values of network proximity given eight sets of AD
genes that we derived from eight distinct information sources (one curated
knowledge based source and seven multi-omic sources,
Table~\ref{tab:genesets}).  This resulted in eight distinct lists of the same
2413 drugs ranked according to increasing network proximity
(Fig.~\ref{fig:workflow}) so that top ranking drugs could be considered for
repurposing.

Although the eight input AD gene sets had little overlap with each other
(Fig.~\ref{fig:workflow}C), the resulting eight lists of network proximity had
markedly high correlations (Fig.~\ref{fig:workflow}D) so that the eight lists
of drug rankings shared substantial similarity.
...
\caption{
  ...
  therefore are likely not efficient for AD.  C) Eight highly dissimilar sets
  of AD genes were input to the workflow.  These eight sets resulted in eight
  lists of the same 2413 drugs ranked according to increasing network
  proximity values.  D) These lists showed high correlation with each other
  relative to the observed dissimilarity of the corresponding AD gene sets.
}
\label{fig:workflow}
\end{figure}
...
sets~(cf.~Fig.~\ref{fig:workflow}C) helps validate our drug
